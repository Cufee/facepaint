...
func BenchmarkRenderCard_NoGlass_200x100(b *testing.B)   { benchCard(b, 200, 100, false) }
func BenchmarkRenderCard_Glass_200x100(b *testing.B)     { benchCard(b, 200, 100, true) }
func BenchmarkRenderCard_NoGlass_1000x1000(b *testing.B) { benchCard(b, 1000, 1000, false) }
func BenchmarkRenderCard_Glass_1000x1000(b *testing.B)   { benchCard(b, 1000, 1000, true) }

func benchCard(b *testing.B, w, h int, glass bool) {
	cardStyle := style.NewStyle(
		style.Parent(style.Style{
			Width:  float64(w),
			Height: float64(h),
			ZIndex: 1,
		}),
		style.SetBorderRadius(float64(w)/8),
		func(s *style.Style) { s.BackgroundColor = color.NRGBA{0, 0, 0, 170} },
	)
	if glass {
		cardStyle = cardStyle.Chain(style.SetBackdrop(NewGlass()))
	}

	card := NewBlocksContent(cardStyle,
		MustNewTextContent(style.NewStyle(
			style.Parent(style.Style{ZIndex: 1}),
			style.SetFont(tests.Font(), color.White),
		), "X"))

	bg := NewEmptyContent(style.NewStyle(
		style.Parent(style.Style{
			Width:    float64(w + 40),
			Height:   float64(h + 40),
			ZIndex:   0,
		...
		func(s *style.Style) { s.BackgroundColor = color.RGBA{80, 80, 80, 255} },
	))
